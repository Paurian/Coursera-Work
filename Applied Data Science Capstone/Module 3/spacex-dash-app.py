...
launch_sites = ["ALL"] + spacex_df['Launch Site'].unique().tolist()
...
# Callback: pie chart showing Success vs. Failed (or successes by site)
@app.callback(
...
def update_success_pie(selected_site):
    ...
    if selected_site == 'ALL':
        ...
        fig = px.pie(
            filtered_df,
            names='class',               # will count 0 vs 1
            title=f'Total Launches for site {selected_site}'
        )
    return fig

# Callback: scatter plot showing correlation between payload and success
@app.callback(
...
def update_payload_scatter(selected_site, payload_range):
    ...
    # Further filter by selected site (unless ALL)
    if selected_site != 'ALL':
    ...
    fig = px.scatter(
        filtered_df,
        x='Payload Mass (kg)',
        y='class',  # 0 or 1 for failure/success
